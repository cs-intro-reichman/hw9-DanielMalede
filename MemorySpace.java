...
public class MemorySpace {
	...
	public int malloc(int length) {
		...
		while (node != null) {
			MemoryBlock freeBlock = node.block;
			...
			if (freeBlock.length >= length) {
				...
				if (freeBlock.length == length) {
					...
					allocatedList.addLast(new MemoryBlock(allocatedAddress, length));
	...
	public void free(int address) {
		...
		Node lastNode = allocatedList.getLast();
		
		while (node != null && node.block.baseAddress != address) {
			if (node == lastNode) break;
			node = node.next;
		}
		
		if (node != null && node.block.baseAddress == address) {
			MemoryBlock blockToFree = node.block;
